...
//// init user analysis objects:
  ...
  for (UInt_t i = 0; i < TNectarRawEvent::fgConfigMdppBoards.size(); ++i)
  {
    ...
    fMdppDisplays.push_back(new TMdppDisplay(uniqueid));
  }
...
while ((pData - pdatastart) < sublen)
{
  ...
  else if (isData)
  {
    ...
    if (isTDC)
    {
      ...
      {
        printf("Warning: MDPP TDC channel info %d out of bounds, maxchannel=%d\n", channel, MDPP_CHANNELS);
  ...
  {
    GO4_STOP_ANALYSIS_MESSAGE("Data error: unknown format in MDPP word 0x%x from module id %d-  stopped!", word, modid);
...
while ((pData - pdatastart) < sublen)
{
  ...
  {
    GO4_STOP_ANALYSIS_MESSAGE("Data error: unknown format in VMMR word 0x%x from module id %d-  stopped!", word, modid);
    return kFALSE;    // redundant, above will do exception
  }
