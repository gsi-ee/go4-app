...
// means maxX/maxY is always upper excluded boundary
  ...
  hBeamMeanCountsX = MakeTH1('I', Form("%s/Mean_Counts_X_G%d_L%d", foldername.Data(), grid, loop),
      Form("X Counts Mean Grid%2d Loop%2d", grid, loop), 6000, -100, 500, "Mean counts");

  hBeamMeanCountsY = MakeTH1('I', Form("%s/Mean_Counts_Y_G%d_L%d", foldername.Data(), grid, loop),
      Form("Y Counts Mean Grid%2d Loop%2d", grid, loop), 6000, -100, 500, "Mean counts");

  hBeamRMSCountsX = MakeTH1('I', Form("%s/RMS_Counts_X_G%d_L%d", foldername.Data(), grid, loop),
      Form("X Counts RMS Grid%2d Loop%2d", grid, loop), 5000, 0, 500, "RMS counts");

  hBeamRMSCountsY = MakeTH1('I', Form("%s/RMS_Counts_Y_G%d_L%d", foldername.Data(), grid, loop),
      Form("Y Counts RMS Grid%2d Loop%2d", grid, loop), 5000, 0, 500, "RMS counts");


  // mean/rms count histogram per wire/qfw channel:
    for(int w=0; w< PEXOR_QFW_WIRES; ++w)
    {
      hBeamMeanCountsGridX[w] = MakeTH1('I', Form("%s/Wire_%d/Mean_Counts_X_G%d_L%d_W%d", foldername.Data(), w, grid, loop, w),
           Form("X Counts Mean Grid%2d Loop%2d Wire%2d", grid, loop, w), 6000, -100, 500, "Mean counts");
      hBeamMeanCountsGridY[w] = MakeTH1('I', Form("%s/Wire_%d/Mean_Counts_Y_G%d_L%d_W%d", foldername.Data(), w, grid, loop, w),
           Form("Y Counts Mean Grid%2d Loop%2d Wire%2d", grid, loop, w), 6000, -100, 500, "Mean counts");
      hBeamRMSCountsGridX[w] = MakeTH1('I', Form("%s/Wire_%d/RMS_Counts_X_G%d_L%d_W%d", foldername.Data(), w,grid, loop, w),
            Form("X Counts RMS Grid%2d Loop%2d Wire%2d", grid, loop, w), 5000, 0, 500, "RMS counts");
      hBeamRMSCountsGridY[w] = MakeTH1('I', Form("%s/Wire_%d/RMS_Counts_Y_G%d_L%d_W%d", foldername.Data(), w,grid, loop, w),
                 Form("Y Counts RMS Grid%2d Loop%2d Wire%2d", grid, loop, w), 5000, 0, 500, "RMS counts");
    }
