...
TTamex_FullProc::TTamex_FullProc(const char* name) : TGo4EventProcessor(name)
{
  ...
  if(GetHistogram("didi")==0)
  {
    ...
    h_7_5_vs_11_9 = MakeTH2 ('I', chis, chead, 550, -200000, 2000000, 550, -20000, 200000);
...
//pl_tmp += 5; // jump over white rabbit time stamps 
    ...
    if (l_first == 1)
    {
      l_pre  = ((l_trg_wind & 0xffff0000) >> 16);  // in ns 
      l_post =  (l_trg_wind & 0x0000ffff);         // in ns
      l_trg_wind_len = l_pre + l_post;
      printf ("trigger window begin:\t%4d ns before trigger \n", l_pre);
      printf ("trigger window end:  \t%4d ns after  trigger \n", l_post);
      printf ("trigger window size: \t%4d ns\n", l_trg_wind_len);
      fflush (stdout);
      l_first = 0;
    }
    ...
    while ( (pl_tmp - pl_se_dat) < (l_dat_len_byte/4) )
    {
      ...
      if ( (l_dat & 0xff) == 0x34) //channel header
      {
        ...
        l_sfp_idx   = (l_cha_head & 0xf000)     >> 12;
          ...
          if (((l_tdc_dat & 0xe0000000) >> 29) == 4)      // tdc channel data
          {
            ...
            for (l_j=0; l_j<MAX_CHA_AN; l_j++)
            {
              if ( (l_ssy_idx == (UInt_t) l_ssy_id[l_j]) && (l_sfp_idx == (UInt_t) l_sfp_id[l_j]) && (l_tam_idx == (UInt_t) l_tam_id[l_j]) && (l_ch_ix == (Int_t) l_cha_id[l_j]) )
              {
                l_ix = l_j;
                l_test_cha_found = 1;
                //printf ("sub sfp tam cha: %2d, %2d, %2d, %2d \n", l_ssy_idx, l_sfp_idx, l_tam_idx, l_ch_ix); 
                break;
              }  
            }
            //printf ("l_ix: %d \n", l_ix);
            ...
            if (l_test_cha_found == 1)
            {
              //printf ("test chan found, l_ix: %d \n", l_ix);
              if ( (l_hct[l_ix] < MAX_HITS) && (l_err_flg == 0) )
              {               
                l_phy_hit_ct[l_ix]++;
                l_tim       [l_ix][l_hct[l_ix]] = l_ch_tim;
                l_coarse    [l_ix][l_hct[l_ix]] = l_coarse_ct;
          ...
          for (l_k=0; l_k<MAX_CHA; l_k++)
          {
            if (l_err_ct [l_h][l_i][l_j][l_k] != 0)
            {
              printf ("SUB: %d, SFP: %d, TAM: %2d, CHA: %2d: \t#errors: %5d \t#hits: %10d \t(%f) \n",
                   l_h, l_i, l_j, l_k, l_err_ct [l_h][l_i][l_j][l_k], l_hit_ct [l_h][l_i][l_j][l_k],
                  (Double_t)l_err_ct [l_h][l_i][l_j][l_k] / (Double_t)l_hit_ct [l_h][l_i][l_j][l_k]);
            }
          }
        }
      }
  ...
  if(fCalibrationDone)
  {
    ...
    for (l_i=0; l_i<MAX_CHA_AN; l_i+=2)
    {
      ...
      {  
	    ...
	    // new JAM2020
	    fOutput->fTimeDiff[l_i]=d_diff;
/////////////////// event filling example end	    
