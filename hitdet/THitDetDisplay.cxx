...
//THitDetDisplay::THitDetDisplay(Int_t deviceid) :
//    fDisplayId(deviceid)
//{
//  ClearBoardDisplays();
//}
//
//THitDetDisplay::~THitDetDisplay()
//{
...
//}
//
//void THitDetDisplay::InitDisplay(Int_t timeslices, Int_t numsnapshots, Bool_t replace)
//{
...
//////////////////////////////////////////////////
//
//THitDetLoopDisplay::THitDetLoopDisplay(Int_t deviceid, Int_t loopid) :
//    THitDetDisplay(deviceid), fLoopId(loopid), fTimeSlices(0)
//{
//}
//
//THitDetLoopDisplay::~THitDetLoopDisplay()
//{
//
//}
//
//void THitDetLoopDisplay::InitDisplay(Int_t timeslices, Bool_t replace)
//{
...
////////////////////////////////////////////////////////////////////////////////////////////////////
//
//THitDetBoardLoopDisplay::THitDetBoardLoopDisplay(Int_t boardid, Int_t loopid) :
//    THitDetLoopDisplay(boardid, loopid), hHitDetRaw(0), hHitDetRawTrace(0)
//{
//  //TString loopname.Form("Board%02d/Loop%02d_", boardid, loopid);
//  //InitDisplay(PEXOR_HitDetSLICES);
//
//}
//
//void THitDetBoardLoopDisplay::InitDisplay(Int_t timeslices, Bool_t replace)
//{
//  THitDetLoopDisplay::InitDisplay(timeslices, replace);
//  if (replace)
//    SetMakeWithAutosave(kFALSE);
//  TGo4Log::Info("THitDetBoardLoopDisplay: Initializing histograms with %d timeslices for Board %d Loop %d", timeslices,
//      GetDevId(), GetLoopId());
//
...
//  obtitle.Form("HitDet Board %d Loop %d Accumulated", brd, loop);
//  int qbins = timeslices * PEXOR_HitDetCHANS;
//  //std::cout <<"*****qbins"<<qbins << std::endl;
//  hHitDetRaw = MakeTH1('I', obname.Data(), obtitle.Data(), qbins, 0, qbins, "HitDet scaler sums");
//
//  obname.Form("Board%d/Loop%d/Brd%d-Loop%d-Trace", brd, loop, brd, loop);
//  obtitle.Form("HitDet Board %d Loop %d Trace", brd, loop);
//  hHitDetRawTrace = MakeTH1('I', obname.Data(), obtitle.Data(), qbins, 0, qbins, "HitDet scalers");
//
//  if (replace)
...
//        hHitDetRaw->GetXaxis()->SetBinLabel(1 + sl * PEXOR_HitDetCHANS + ch, binlabel.Data());
//        hHitDetRawTrace->GetXaxis()->SetBinLabel(1 + sl * PEXOR_HitDetCHANS + ch, binlabel.Data());
//      }
//    }
//  }
//}
  ...
  fDisplayId(boardid)
{
  ...
  //InitDisplay(HitDet_TRACEBINS, HitDet_MAXSNAPSHOTS);
}
...
void THitDetBoardDisplay::InitDisplay(Int_t tracelength, Int_t numsnapshots, Bool_t replace)
{
    ...
    for(Int_t ch=0; ch<HitDet_CHANNELS; ++ch)
    {
      ...
      obname.Form("Board%d/Channel%d/SumTrace_%d__%d", brd, ch, brd, ch);
    ...
    Int_t totlength=tracelength * numsnapshots; // JAM todo: different variable for stitched length here later
...
//THitDetGridLoopDisplay::THitDetGridLoopDisplay(Int_t gridid, Int_t loopid) :
//    THitDetLoopDisplay(gridid, loopid), hBeamXSlice(0), hBeamYSlice(0), hBeamXSliceOffs(0), hBeamYSliceOffs(0),
...
//
////***********************************************************
//THitDetGridLoopDisplay::~THitDetGridLoopDisplay()
//{
//
//}
//void THitDetGridLoopDisplay::SetGridEvent(THitDetProfileEvent* out)
//{
//  fGridData = out->GetGrid(GetDevId());
//
//}
//
//void THitDetGridLoopDisplay::InitDisplay(int timeslices, Bool_t replace)
//{
//  TGo4Log::Info("THitDetGridLoopDisplay: Initializing histograms with %d timeslices for Grid %d Loop %d", timeslices,
//      GetDevId(), GetLoopId());
//  TString obname;
//  TString obtitle;
//  TString foldername;
...
//  Int_t wiresX = PEXOR_HitDet_WIRES;
//  Int_t wiresY = PEXOR_HitDet_WIRES;
//  Int_t gix = -1;
//  if (fGridData)
//  {
...
//      for(int w=0; w< PEXOR_HitDet_WIRES; ++w)
//      {
...
//    Double_t xposition[PEXOR_HitDet_WIRES];
//    for (Int_t ix = 0, jx = minX; (ix < binsX) && (jx + 1 < PEXOR_HitDet_WIRES); ++ix, ++jx)
//    {
...
//    }
//
//    Double_t yposition[PEXOR_HitDet_WIRES];
//    for (Int_t ix = 0, jx = minY; (ix < binsY) && (jx + 1 < PEXOR_HitDet_WIRES); ++ix, ++jx)
//    {
...
//}
//
//void THitDetGridLoopDisplay::AdjustDisplay(THitDetLoop* loopdata)
//{
...
/////*********************************************
//
//THitDetGridDisplay::THitDetGridDisplay(Int_t gridid) :
//    THitDetDisplay(gridid), hBeamX(0), hBeamY(0), hBeamAccX(0), hBeamAccY(0), hPosX(0), hPosY(0), hPosAccX(0), hPosAccY(0),
...
//{
//
//  for (int i = 0; i < PEXOR_HitDetLOOPS; ++i)
//  {
//    AddLoopDisplay(new THitDetGridLoopDisplay(gridid, i));
//  }
//  InitDisplay(PEXOR_HitDetSLICES);
//}
//
////***********************************************************
//THitDetGridDisplay::~THitDetGridDisplay()
//{
//
//}
//
//THitDetGridLoopDisplay* THitDetGridDisplay::GetLoopDisplay(Int_t index)
//{
//  return (THitDetGridLoopDisplay*) GetSubDisplay(index);
//}
//
//void THitDetGridDisplay::SetGridEvent(THitDetProfileEvent* out)
//{
//  fGridData = out->GetGrid(GetDevId());
//  for (unsigned i = 0; i < fLoops.size(); ++i)
//  {
//    THitDetGridLoopDisplay* cuploop = dynamic_cast<THitDetGridLoopDisplay*>(fLoops[i]);
//    cuploop->SetGridEvent(out);
//  }
//
//}
//
//void THitDetGridDisplay::SetProfileParam(THitDetProfileParam* par)
//{
//  fParam = par;
//  for (unsigned i = 0; i < fLoops.size(); ++i)
//  {
//    THitDetGridLoopDisplay* loop = dynamic_cast<THitDetGridLoopDisplay*>(fLoops[i]);
//    loop->SetProfileParam(par);
//  }
//}
//
//void THitDetGridDisplay::InitDisplay(int timeslices, Bool_t replace)
//{
//  THitDetDisplay::InitDisplay(timeslices, replace);    // handle initialization of subloops
//  if (replace)
//    SetMakeWithAutosave(kFALSE);
//  TString obname;
//  TString obtitle;
...
//}
//
//void THitDetGridDisplay::AdjustDisplay(THitDetBoard* boarddata)
//{
//
...
//}
//
//THitDetCupLoopDisplay::THitDetCupLoopDisplay(Int_t cupid, Int_t loopid) :
//    THitDetLoopDisplay(cupid, loopid), fCupData(0), hCupSlice(0), hCupSliceOffs(0), hAccCupSlice(0),
...
//
//}
//THitDetCupLoopDisplay::~THitDetCupLoopDisplay()
//{
//
//}
//void THitDetCupLoopDisplay::SetCupEvent(THitDetProfileEvent* out)
//{
//  fCupData = out->GetCup(GetDevId());
//
//}
//
//void THitDetCupLoopDisplay::InitDisplay(Int_t timeslices, Bool_t replace)
//{
//// TODO: put some scaler histograms here
//
//  TGo4Log::Info("THitDetCupLoopDisplay: Initializing histograms with %d timeslices for Cup %d Loop %d", timeslices,
//      GetDevId(), GetLoopId());
//  if (replace)    //TGo4Analysis::Instance()->
//    SetMakeWithAutosave(kFALSE);
//  THitDetLoopDisplay::InitDisplay(timeslices, replace);    // important to remember new timeslices!
//  Int_t cup = GetDevId();
//  Int_t loop = GetLoopId();
//  Int_t segments = PEXOR_HitDet_CUPSEGMENTS;
//  TString foldername;
//  if (fCupData)
//  {
...
//}
//
//void THitDetCupLoopDisplay::AdjustDisplay(THitDetLoop* loopdata)
//{
//
//  // check if we have different timeslices:
//
...
///////////////////////////////////////////////////////////////////////////////7
//
//THitDetCupDisplay::THitDetCupDisplay(Int_t cupid) :
//    THitDetDisplay(cupid), hCupScaler(0), hCupAccScaler(0), fCupData(0)
//{
//  for (unsigned i = 0; i < PEXOR_HitDetLOOPS; ++i)
//  {
//    AddLoopDisplay(new THitDetCupLoopDisplay(cupid, i));
//  }
//  InitDisplay(PEXOR_HitDetSLICES);
//}
//THitDetCupDisplay::~THitDetCupDisplay()
//{
//
//}
//void THitDetCupDisplay::SetCupEvent(THitDetProfileEvent* out)
//{
//  fCupData = out->GetCup(GetDevId());
//  for (unsigned i = 0; i < fLoops.size(); ++i)
//  {
//    THitDetCupLoopDisplay* cuploop = dynamic_cast<THitDetCupLoopDisplay*>(fLoops[i]);
//    cuploop->SetCupEvent(out);
//  }
//
//}
//THitDetCupLoopDisplay* THitDetCupDisplay::GetLoopDisplay(Int_t index)
//{
//  return (THitDetCupLoopDisplay*) GetSubDisplay(index);
//}
//
//void THitDetCupDisplay::InitDisplay(Int_t timeslices, Bool_t replace)
//{
//  THitDetDisplay::InitDisplay(timeslices, replace);
//// TODO: put some scaler histograms here
//TString foldername;
//  Int_t cup = GetDevId();
//  Int_t segs = PEXOR_HitDet_CUPSEGMENTS;
//  if (fCupData)
//  {
//    // take real number of segments from event object
...
//}
//
//void THitDetCupDisplay::AdjustDisplay(THitDetBoard* boarddata)
//{
//
////   Bool_t dostop=kFALSE;
//  //Double_t mtime=boarddata->fQfwLoopTime * 20 / 1000; // measurement time in us
